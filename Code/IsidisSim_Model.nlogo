...
to create-goal-gradient                                                        ;; setup procedure that creates gradeint from Serapeum to seafront
  ...
  if Harbour = true [
    let f-goal patch -568 -26                                                  ;; particular point on the harbour chosen as the ending destination. This can be adjusted
    ask patches with [street-nw]
      [set distance-goal distance f-goal]
    set destination f-goal]
end
...
Try adjusting the processional destination to see how routes change

## EXTENDING THE MODEL

Add in more complex spectator dynamics so they are not static agents
